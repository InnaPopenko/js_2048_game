...
class Game {
    ...
    addRandomTile() {
        ...
        for(let rowThree = 0; rowThree < this.size; rowThree++){
            for(let colThree = 0; colThree < this.size; colThree++)if (this.board[rowThree][colThree] === 0) emptyCells.push({
                rowThree,
                colThree
            });
        }
        if (emptyCells.length === 0) return;
        const { rowThree: r, colThree: c } = emptyCells[Math.floor(Math.random() * emptyCells.length)];
    ...
    moveLeft() {
        ...
        const newBoard = this.board.map((row)=>{
            ...
            while(compressed.length < this.size)compressed.push(0);
            if (!moved && compressed.some((v, i)=>v !== row[i])) moved = true;
            return compressed;
        });
        if (moved) {
            this.board = newBoard;
        ...
        if (this.checkWin()) this.status = "won";
        else if (this.checkLose()) this.status = "lost";
    }
    moveRight() {
    ...
    rotateClockwise(board) {
        ...
        for(let r = 0; r < this.size; r++)for(let c = 0; c < this.size; c++)newBoard[c][this.size - 1 - r] = board[r][c];
        return newBoard;
    }
    rotateCounterClockwise(board) {
        const newBoard = this.createEmptyBoard();
        for(let rolSecond = 0; rolSecond < this.size; rolSecond++)for(let colSecond = 0; colSecond < this.size; colSecond++)newBoard[this.size - 1 - colSecond][rolSecond] = board[rolSecond][colSecond];
        return newBoard;
    }
    checkWin() {
    ...
    checkLose() {
        ...
        for(let row = 0; row < this.size; row++)for(let col = 0; col < this.size; col++){
            ...
            if (col < this.size - 1 && this.board[row][col + 1] === val) return false;
            if (row < this.size - 1 && this.board[row + 1][col] === val) return false;
        }
        return true;
    }
}
